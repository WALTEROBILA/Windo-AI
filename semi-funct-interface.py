...
#Upload files
cover_letter_file = st.file_uploader("Upload an Existing Cover Letter", type=["pdf","docx"])
resume_file = st.file_uploader("Upload an Existing Resume Resume", type=["pdf","docx"])
...
if generate:
    ...
    - Ensure you have listed relevant competencies, as wells as the skills involved.
    - The core competencies section can't be empty, can't be generic. Ensure it is convincingly populated.
    - Do not return "categoryskills" under the core competencies. Ensure the section is well populated.
    - The core competencies section has to have the requisite skills.
    ...
    with st.spinner("Generating your documents..."):
        ...
        st.session_state.cover_letter_doc = save_cover_letter_to_word(
            applicant_header=result["applicant_header"],
            company_header=result["company_header"],
            cover_letter=result["cover_letter"],
            applicant_name=candidate_profile.get("name", "")
        ...
        st.session_state.resume_doc = save_resume_to_word(
            resume_profile=result["resume_profile"],
            summary=result["resume_summary"],
            competencies=result["resume_core_competencies"],
            experience=result["resume_experience"],
...
if "cover_letter_doc" in st.session_state:
    st.download_button(
        label="📄 Download Cover Letter",
        data=st.session_state.cover_letter_doc,
        file_name="Cover_Letter.docx",
        mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document"
    )

if "resume_doc" in st.session_state:
    st.download_button(
        label="📄 Download Resume",
        data=st.session_state.resume_doc,
        file_name="Resume.docx",
        mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document"
    )
